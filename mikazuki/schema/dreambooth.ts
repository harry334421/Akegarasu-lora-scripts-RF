Schema.intersect([
    ...
    Schema.object({
        ...
        caption_tag_dropout_rate: Schema.number().min(0).max(1).step(0.01).description("按逗号分隔的标签来随机丢弃 tag 的概率"),
        ...
        Schema.intersect([
            ...
            Schema.union([
                Schema.object({
                    ...
                    vae_custom_scale: Schema.number().step(0.0001).description("VAE 自定义缩放"),
                    vae_custom_shift: Schema.number().step(0.0001).description("VAE 自定义偏移"),
                }),
                Schema.object({}),
            ]),
