...
use_flow_model=0                 # 启用 Rectified Flow 训练目标 | enable Rectified Flow training objective
flow_use_ot=0                    # 使用余弦最优传输配对 | use cosine optimal transport pairing
flow_timestep_distribution="logit_normal" # 时间步分布: logit_normal 或 uniform | timestep distribution
flow_uniform_static_ratio=""     # 固定的时间步偏移比率 (例如 2) 留空不使用 | fixed timestep shift ratio (e.g. 2), leave empty to disable
