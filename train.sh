...
pretrained_model="F:/下载资源/Illustrious-XL-v2.0.safetensors" # base model path | 底模路径
...
train_data_dir="D:/[01]-Entertainment/[01]-AITest/训练集等/rurudo-traindata-20250120/train" # train dataset path | 训练数据集路径
...
resolution="1536,1536"  # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
batch_size=1          # batch size
max_train_epoches=11  # max train epoches | 最大训练 epoch
save_every_n_epochs=2 # save every n epochs | 每 N 个 epoch 保存一次

train_unet_only=0            # train U-Net only | 仅训练 U-Net，开启这个会牺牲效果大幅减少显存使用。6G显存可以开启
train_text_encoder_only=0    # train Text Encoder only | 仅训练 文本编码器
stop_text_encoder_training=0 # stop text encoder training | 在第N步时停止训练文本编码器
protected_tags_file="F:/Software/lora-scripts/config/protected.txt"
...
sample_prompts="F:/Software/lora-scripts/config/sample_prompts.txt"
sample_sampler="euler_a"
# Rectified Flow 设置 | Rectified Flow settings
use_flow_model=1                 # 启用 Rectified Flow 训练目标 | enable Rectified Flow training objective
flow_use_ot=0                    # 使用余弦最优传输配对 | use cosine optimal transport pairing
flow_timestep_distribution="uniform" # 时间步分布: logit_normal 或 uniform | timestep distribution
flow_uniform_static_ratio="2"     # 固定的时间步偏移比率 (例如 2) 留空不使用 | fixed timestep shift ratio (e.g. 2), leave empty to disable

# Learning rate | 学习率
lr="2e-6"                           # learning rate | 学习率，在分别设置下方 U-Net 和 文本编码器 的学习率时，该参数失效
unet_lr="2e-6"                      # U-Net learning rate | U-Net 学习率
learning_rate_te1="4e-7"
learning_rate_te2="5e-7"
text_encoder_lr="5e-7"              # Text Encoder learning rate | 文本编码器 学习率
lr_scheduler="cosine_with_restarts" # "linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup", "adafactor"
lr_warmup_steps=1900                 # warmup steps | 学习率预热步数，lr_scheduler 为 constant 或 adafactor 时该值需要设为0。
lr_restart_cycles=4              # cosine_with_restarts restart cycles | 余弦退火重启次数，仅在 lr_scheduler 为 cosine_with_restarts 时起效。

# Optimizer settings | 优化器设置
optimizer_type="Lion" # Optimizer type | 优化器类型 默认为 AdamW8bit，可选：AdamW AdamW8bit Lion Lion8bit SGDNesterov SGDNesterov8bit DAdaptation AdaFactor prodigy

# Output settings | 输出设置
output_name="rd-rf"           # output model name | 模型保存名称
...
max_bucket_reso=2880            # arb max resolution | arb 最大分辨率
...
if [[ $use_flow_model == 1 ]]; then
  extArgs+=("--flow_model")
  extArgs+=("--flow_timestep_distribution $flow_timestep_distribution")
  if [[ $flow_use_ot == 1 ]]; then extArgs+=("--flow_use_ot"); fi
  if [[ $flow_uniform_static_ratio ]]; then extArgs+=("--flow_uniform_static_ratio $flow_uniform_static_ratio"); fi
fi
